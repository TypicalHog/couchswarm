...
export default function CouchSwarm() {
  ...
  return <div className="shell">
    ...
    <Dialog open={modal === 'invite'} onOpenChange={open => !open && setModal(null)}><DialogContent className="modal" finalFocus={() => openerRef.current?.isConnected ? openerRef.current : inviteRef.current}><DialogHeader><DialogTitle>Save them a seat.</DialogTitle><DialogDescription>Send this link to your people. Everyone who has it can join your room.</DialogDescription></DialogHeader><label htmlFor="invite-link">Your room’s invite link</label><div className="invite-link"><input id="invite-link" className="text-input" readOnly value={swarm.inviteUrl} onFocus={e => e.target.select()}/><button className="primary-button" onClick={() => void copyInvite()}>{copied ? <Check size={16}/> : <Copy size={16}/>} {copied ? 'Copied' : 'Copy'}</button></div>{copyError && <div className="error" role="alert">{copyError}</div>}{swarm.error && <div className="error" role="alert">{swarm.error}</div>}{isHost && <button className="quiet-button" aria-busy={swarm.busy} onClick={() => { if (swarm.busy) return; setCopied(false); setCopyError(''); setRotated(false); void swarm.control('rotate').then(ok => setRotated(ok)); }}>Create a new invite link</button>}<p className="help-limit"><output>{rotated ? 'New link created. The old link no longer works, so copy this one. ' : ''}</output>Up to {MAX_SEATS} people. Your room stays open for {ROOM_TTL_MS / 3600000} hours after you create it. The host keeps control of play, pause, and seeking. Creating a new link empties the couch — everyone else rejoins with the new link — and reissues host control to this device.</p></DialogContent></Dialog>

...
export default function CouchSwarm() {
  ...
  const announcement = hasSource && swarm.countdown > 0 && swarm.armed ? `Starting in ${swarm.countdown}` : swarm.seatLost ? 'Your seat was taken. Waiting for a free seat.' : !swarm.connected && session ? (room ? 'Connection lost, reconnecting' : 'Connecting to the room') : swarm.error || room?.reason || '';
  const hostAway = hasSource && !swarm.hostPresent;
  ...
  async function invite() {
    ...
    setCopied(false); setCopyError(''); setRotated(false);
    setModal('invite');
  }
  ...
  return <div className="shell">
    ...
    <Dialog open={modal === 'invite'} onOpenChange={open => !open && setModal(null)}><DialogContent className="modal" finalFocus={() => openerRef.current?.isConnected ? openerRef.current : inviteRef.current}><DialogHeader><DialogTitle>Save them a seat.</DialogTitle><DialogDescription>{swarm.inviteStale ? 'The link this tab is holding has been replaced.' : 'Send this link to your people. Everyone who has it can join your room.'}</DialogDescription></DialogHeader>{swarm.inviteStale ? <p className="help-limit">Your host created a new invite link, so the one you joined with no longer lets anybody in. Ask them for the current one.</p> : <><label htmlFor="invite-link">Your room’s invite link</label><div className="invite-link"><input id="invite-link" className="text-input" readOnly value={swarm.inviteUrl} onFocus={e => e.target.select()}/><button className="primary-button" onClick={() => void copyInvite()}>{copied ? <Check size={16}/> : <Copy size={16}/>} {copied ? 'Copied' : 'Copy'}</button></div></>}{copyError && <div className="error" role="alert">{copyError}</div>}{swarm.error && <div className="error" role="alert">{swarm.error}</div>}{isHost && <button className="quiet-button" aria-busy={swarm.busy} onClick={() => { if (swarm.busy) return; setCopied(false); setCopyError(''); setRotated(false); void swarm.control('rotate').then(ok => setRotated(ok)); }}>Create a new invite link</button>}<p className="help-limit"><output>{rotated ? 'New link created. The old link no longer works, so copy this one. ' : ''}</output>Up to {MAX_SEATS} people. {isHost ? `Your room stays open for ${ROOM_TTL_MS / 3600000} hours after you create it.` : `The room stays open for ${ROOM_TTL_MS / 3600000} hours after the host creates it.`} The host keeps control of play, pause, and seeking.{isHost && ' Creating a new link empties the couch — everyone else rejoins with the new link — and reissues host control to this device.'}</p></DialogContent></Dialog>
    <Dialog open={modal === 'source'} onOpenChange={open => !open && setModal(null)}><DialogContent className="modal" initialFocus={magnetRef} finalFocus={() => openerRef.current?.isConnected ? openerRef.current : changeRef.current}><DialogHeader><DialogTitle>What are we watching?</DialogTitle><DialogDescription>Paste another magnet or HTTPS .torrent link below. Loading it starts a fresh buffer for everyone.</DialogDescription></DialogHeader>{torrentForm}<p className="help-limit">Use a magnet or HTTPS .torrent link with browser-compatible video and WebRTC seeders or web seeds.</p></DialogContent></Dialog>

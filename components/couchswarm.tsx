...
export default function CouchSwarm() {
  ...
  const displayMembers = session ? members.slice().sort((a, b) => Number(b.id === room?.hostId) - Number(a.id === room?.hostId) || Number(b.id === session.memberId) - Number(a.id === session.memberId)) : [{ id: 'you', name: 'You', ready: false, buffered: 0, progress: 0, epoch: -1, lastSeen: 0 }];
  ...
  const announcement = hasSource && swarm.countdown > 0 && swarm.armed ? `Starting in ${swarm.countdown}` : !swarm.connected && session ? 'Connection lost, reconnecting' : room?.reason || '';
      ...
      <div className="workspace"><section aria-label="Room player">
        <div className="player-card" ref={playerRef} onPointerMove={revealControls}>
          <div className="screen">
            {/* eslint-disable-next-line jsx-a11y/media-has-caption -- this player has no text tracks by design; subtitles are unsupported. */}
          ...
          <div className="player-controls">
            <div className="timeline"><span className="time">{time(seek ?? swarm.playhead)}</span><Slider aria-label="Playback position" value={[seek ?? swarm.playhead]} min={0} max={swarm.duration || 1} step={1} disabled={seekLost || swarm.busy} onValueChange={value => setSeek(Array.isArray(value) ? value[0] : value)} onValueCommitted={(value, details) => { const position = Array.isArray(value) ? value[0] : value; const send = () => void swarm.control('seek', { position }).finally(() => setSeek(null)); clearTimeout(seekTimer.current); if (details.reason === 'keyboard') seekTimer.current = setTimeout(send, 400); else send(); }}/><span className="time">{swarm.duration ? time(swarm.duration) : '--:--'}</span></div>
            <div className="control-row"><div className="control-group"><button className="play-button" disabled={!canPlay} aria-describedby="play-hint" onClick={() => { if (Date.now() - lastToggle.current < 400) return; lastToggle.current = Date.now(); void swarm.control(isPlaying ? 'pause' : 'play'); }}>{isPlaying ? <Pause size={15} fill="currentColor"/> : <Play size={15} fill="currentColor"/>}{isPlaying ? 'Pause for all' : 'Play for all'}</button><div className="volume-control" onMouseEnter={() => setVolumeOpen(true)} onMouseLeave={() => setVolumeOpen(false)} onBlur={e => { if (!e.currentTarget.contains(e.relatedTarget as Node | null)) setVolumeOpen(false); }}><button className="icon-button" aria-label={muted ? 'Unmute' : 'Mute'} title="Click to mute; volume slider opens above" onClick={() => { if (!muted) { setMuted(true); return; } const v = volume > 0 ? volume : .5; setVolume(v); if (videoRef.current) videoRef.current.volume = v; setMuted(false); }} onFocus={() => setVolumeOpen(true)}>{muted ? <VolumeX size={19}/> : <Volume2 size={19}/>}</button>{volumeOpen && <div className="volume-slider"><Slider aria-label="Your volume" value={[muted ? 0 : volume]} min={0} max={1} step={.01} onValueChange={value => { const v = Array.isArray(value) ? value[0] : value; setVolume(v); setMuted(v === 0); if (videoRef.current) videoRef.current.volume = v; }}/></div>}</div><span className="control-hint" id="play-hint"><Crown size={13}/>{playHint}</span></div><div className="control-group"><span className="pill"><span className={`dot ${swarm.everyoneReady ? 'live' : ''}`}/>{!hasSource ? 'Waiting for a movie' : !swarm.connected ? 'Reconnecting' : isPlaying ? 'In sync' : swarm.everyoneReady ? 'Everyone ready' : 'Buffering'}</span><button className="icon-button" aria-label={isFullscreen ? 'Exit fullscreen' : 'Enter fullscreen'} aria-pressed={isFullscreen} onClick={() => void fullscreen()}><Maximize size={18}/></button></div></div>
          </div>
        ...
        {media.files.length > 1 && <div className="video-selection"><label id="video-file-label" htmlFor="video-file">Video in this torrent</label><Select value={String(room?.fileIndex || 0)} onValueChange={value => { if (swarm.busy) return; const next = Number(value); if (value !== null && next !== (room?.fileIndex || 0)) void swarm.control('file', { fileIndex: next }); }} disabled={!isHost}><SelectTrigger id="video-file" aria-labelledby="video-file-label" className="w-full"><SelectValue>{media.files[room?.fileIndex || 0]?.path}</SelectValue></SelectTrigger><SelectContent>{media.files.map((file, i) => <SelectItem key={i} value={String(i)}>{file.path}</SelectItem>)}</SelectContent></Select></div>}

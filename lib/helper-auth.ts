...
export async function roomAccess(request: Request, id: string) {
  ...
  const db = getDb();
  const member = await db.prepare('SELECT id FROM members WHERE room_id = ? AND token_hash = ?').bind(id, await hash(token)).first<{ id: string }>();
  ...
  return { db, room, memberId: member.id };
}

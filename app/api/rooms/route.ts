...
async function handler(request: Request) {
  ...
  await db.batch([
    ...
    db.prepare('INSERT INTO rooms (id, host_id, invite_hash, host_key_hash, source, reason, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)')
      .bind(id, memberId, inviteHash, hostKeyHash, source, source ? 'Buffering a new movie.' : 'Waiting for a movie.', now),
